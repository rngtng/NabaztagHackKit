...
- **Role:** `task firmware:test` — compiles the **firmware's own C VM** natively (no
...
| `tools/mockserver/` | written for this repo | stdlib-only Python mock HTTP file server (#39) — logs requests, serves an app's `src/<app>/assets/` so the sim can fetch `init.forth`/`bc.jsp`/`*.mp3` end-to-end (`task app-piper:simulate:mock`) |
