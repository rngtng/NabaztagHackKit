...
| `src/firmwareV2/` | original to this repo; ARM7TDMI startup/linker/register base **copied from `src/firmware/` @ `3a37cef`** | eLua-successor Lua 5.4 port to the V2 (ML67Q4051), issue #87. `sys/{ml67q4051.ld,asm/*.s,src/irq.c,inc/*.h}` and `inc/common.h` are verbatim copies of the vendored `nabgcc` startup, so a VM/register fix in `src/firmware` may apply here too (grep the sibling). Local change: `common.h` drops the `utils/debug.h` (UART) include - the V2 board has no UART. The Lua runtime itself will be vendored at M4 (#92). |
