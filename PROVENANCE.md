...
| `src/boot/` | the original Violet/IAR boot (via `firmware_nabaztag`), split into modules | recovery path — WiFi provisioning + `bc.jsp` fetch. Device build's TCP/IP/DHCP/DNS/HTTP now compose the shared `lib/net` stack (#47), driven from boot's loop via `lib/sys/task`; `ipv4.mtl`/`dns.mtl`/`http.mtl` are thin wrappers that alias boot's historical names. `config.mtl` + `wifi.mtl` stay boot-local; the `SIMU` build keeps boot's own resolver/client over `tcpudp_emu.mtl` |
