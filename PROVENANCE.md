...
| `lua/firmware/lua/` | PUC-Rio **Lua 5.4.7** ([lua/lua](https://github.com/lua/lua), `lua.org/ftp`) | Vendored interpreter, whole `src/` verbatim. **Local edits** for the 124 KB / no-FPU target: `luaconf.h` (`LUA_32BITS=1` + off-newlib number/console overrides, #106/#133; non-variadic `luai_num2str` + fixed `'.'` radix, #213), `lbaselib.c` (drop `dofile`/`loadfile`), the parser-less image (#128) - `ldo.c` (`f_parser` text branch) + `lstate.c` (`luaX_init` call) guard the only two references to the dropped parser/lexer behind `-DLUA_NOPARSER`, so `lparser`/`llex`/`lcode` need not be linked - and the double-soft-float scavenge (#213, all behind the same `-DLUA_NOPARSER`): `lapi.c` (`lua_dump` -> error; `ldump.c` unlinked), `lstrlib.c` (`string.dump` + `string.pack`'s C-double `'d'` dropped, non-variadic float `%f/%e/%g`), `lauxlib.c` (`luaL_tolstring` numbers without `%f`, int version-mismatch message), `lobject.c` (`luaO_pushvfstring` `%f` case out). License **MIT**. |
| `lua/firmware/` | original to this repo; ARM startup/linker/registers copied from `mtl/firmware` @ `3a37cef` | Bare-metal Lua 5.4 port (#87). HAL drivers (led, spi, audio, i2c, rfid, motor, usb, config) are verbatim/trimmed ports of `mtl/firmware`'s - a register fix there may apply here, grep the sibling. Per-driver detail: #89/#116/#117/#118/#123/#143; `hal/config.c` (#214) is `mtl/firmware/src/utils/mem.c`'s `write_uc_flash_sec`/`read_uc_flash` reduced to a fixed-layout record in the one config sector (no caller-supplied shadow buffer / index / length). The `src/net`/`src/usb` RT2501 802.11 stack diverges from `mtl/firmware`'s: **WPA2-CCMP only** (#124) - `net/rc4.*` deleted, MD5 dropped from `net/hash.c`, WEP/WPA1/TKIP paths stripped from `net/{eapol,ieee80211}.c` - so a WEP/TKIP-touching fix on the mtl twin does not apply here. |
| `mtl/tools/openocd/`, `lua/tools/openocd/` | original to this repo | JTAG debrick configs (Raspberry Pi bit-bang) - the host-side flashing exception (see CLAUDE.md). `lua/tools/openocd/patches/ml67q40xx-tail-drop.patch` is our local fix on top of the downloaded RedoX `openocd_0.8.0_oki.patch`: its `ml67q40xx` block-write programmed `count/4` words and dropped a trailing partial word (#203). Applied on the Pi, not vendored source. |
