...
| `src/app-template/forth_time.mtl` | the time subset of `src/app-piper/forth/misc.mtl` | `time&date local>string utc>string time-ms uptime ms update-time time?` — verbatim wrappers over `lib/sys/time.mtl`, plus a `#ifdef SIMU` `ntp_get_time` stub (the simulator transport has no UDP/DNS stack). Copyright (c) 2025 Andrea Bonomi, MIT License (see file header) |
| `src/app-template/forth_task.mtl` | verbatim copy of `src/app-piper/forth/task.mtl` | `task-start task-stop task-suspend task-resume .tasks` — wrappers over `lib/sys/task.mtl`'s scheduler (already run by `main.mtl` via `loopcb #task_scheduler`); dropped the `#include "lib/protos/task_protos.mtl"` line since `main.mtl` pulls `lib/sys/task.mtl` in ahead of `app.mtl`. Copyright (c) 2025 Andrea Bonomi, MIT License (see file header) |
