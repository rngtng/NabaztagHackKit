 ...
 * main loop. A set_led() or another led_fade() on the same LED replaces it.
 *
...
void led_fade(uint8_t led,uint32_t color,uint32_t ms)
{
  ...
  led_fade_len[phys]=ms;
}

/**
 * @brief Advance running fades; call from the main loop
 *
 * Rate-limited to one interpolation step - and at most one SPI flush for
 * all LEDs together - per LED_FADE_TICK_MS.
 */
