...
{
#if defined(DEBUG) || defined(DEBUG_WIFI) || defined(DEBUG_USB)
  int32_t i,j;
  uint8_t buffer[64];
  consolestr(EOL);
  for(i=0;i<len;i+=16)
...
#else
  /* Debug-only hex dump: no-op (and no UART side-channel) in release. */
  (void)src; (void)len;
#endif
}
...
//        int sendarp=0;
  ...
  /* Seed libc rand() once at boot from ADC noise + the free-running timer.
     rand() backs the EAPOL SNonce and WEP IVs; it was never seeded, so it
     produced an identical sequence every boot (the VM's sysRand is a
     separate generator). get_adc_value()'s low bits carry analog noise. */
  {
    uint32_t seed = counter_timer;
    for(iii=0;iii<32;iii++)
      seed = (seed << 1) ^ get_adc_value();
    srand(seed);
  }
