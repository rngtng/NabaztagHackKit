...
PHCD_TD hcd_control_transfer_done(PHCD_TD td)
{
  ...
  /* Capture the done-queue link before any free: hcd_delete_td() may
     reclaim td (it walks HeadP->TailP and frees the failed TD). */
  ...
  if(cc)
  {
    ...
    {
      hcd_delete_td(ed);   /* frees td and unlinks td->list */
      hcd_pause_ed(ed);
      return td_next;      /* td already reclaimed: do not free again */
    }
  }
...
PHCD_TD hcd_bulk_transfer_done(PHCD_TD td)
{
  ...
  if((cc != 0) && (cc != CC_DATAOVERRUN) && (cc != CC_DATAUNDERRUN))
  {
    ...
    if( ((ed->HcED.HeadP & HcED_HeadP_HALT) != 0) &&
        ((ed->HcED.HeadP & 0xFFFFFFF0) != 0) )
    {
      hcd_delete_td((PHCD_ED)ed);   /* frees td and unlinks td->list */
      hcd_pause_ed(ed);
      td_reclaimed = 1;
    }
  }
  ...
  if(!td_reclaimed)   /* td already freed by hcd_delete_td: do not free again */
  {
    list_del(&td->list);
    hcd_free(td);
  }
