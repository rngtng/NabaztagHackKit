...
int32_t hcd_malloc_rest(uint8_t Bank)
{
  ...
  {
    uint32_t used = LastBufferAddress - Buffer->Address;
    FreeMaxSize = (Buffer->Size > used) ? (Buffer->Size - used) : 0;
  }

  rest = max(Gap2MaxSize, FreeMaxSize);
  rest = (rest > sizeof(MMDL)) ? (rest - sizeof(MMDL)) : 0;
  return (int32_t)rest;
}
