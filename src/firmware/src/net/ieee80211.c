      ...
      DBG_WIFI("Probe_Req"EOL);
			if(ieee80211_mode == IEEE80211_M_MASTER) {
				...
				while(frame_current + 2 <= frame_end) {
					if(frame_current + 2 + frame_current[1] > frame_end) break;
					if(frame_current[0] == IEEE80211_ELEMID_SSID) {
						uint8_t ssidlen = frame_current[1];
						if(ssidlen > IEEE80211_SSID_MAXLEN) ssidlen = IEEE80211_SSID_MAXLEN;
						ssid_present = 1;
						for(i=0;i<ssidlen;i++)
...
//                   dump(frame,length);
			if(ieee80211_mode == IEEE80211_M_MANAGED) {
              ...
              case IEEE80211_ELEMID_RSN:
              {
                ...
                if(current + 2 > frame_end) break;
                count = (current[0] << 0)|(current[1] << 8);
                current += 2;
                found = 0;
                scan_result.encryption = 0;
                for(i=0;i<count && current + IEEE80211_OUI_LEN <= frame_end;i++) {
...
//#pragma pack()
    ...
    case IEEE80211_CRYPT_WPA2:
			ieee80211_authmode = IEEE80211_AUTH_OPEN;
			memcpy(ieee80211_key, key, EAPOL_MASTER_KEY_LENGTH);
			rt2501_set_key(0, NULL, NULL, NULL, RT2501_CIPHER_NONE);
			eapol_init();
			break;
		default:
