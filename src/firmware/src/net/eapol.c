...
/**
 * @brief Derive the 32-byte WPA PMK from passphrase + SSID (PBKDF2-SHA1).
 *
 * @param [in]  password    Buffer for the password
 * @param [in]  ssid        Buffer of the SSID
 * @param [in]  ssidlength  Length of the SSID
 * @param [out] pmk         Output buffer for the PMK (32 bytes)
 */
static void password_to_pmk(const uint8_t *password,
                            uint8_t *ssid, uint16_t ssidlength,
                            uint8_t *pmk)
{
	/* PBKDF2 emits two 20-byte SHA1 blocks (40 bytes total); only the first
	   32 are the PMK. Assemble in a local scratch so callers need a 32-byte
	   buffer, not 40 (the old code wrote pmk[0..39], overflowing pmk[32]). */
	uint8_t dk[40];
	F(password, strlen((char*)password), ssid, ssidlength, 4096, 1, dk);
	F(password, strlen((char*)password), ssid, ssidlength, 4096, 2, dk+20);
	memcpy(pmk, dk, EAPOL_MASTER_KEY_LENGTH);
}
...
void eapol_init(void)
{
  ...
  DBG_WIFI(dbg_buffer);
	/* The PMK is pre-derived (PBKDF2 over passphrase+SSID) by the VM and
	   handed to us as the 32-byte key in ieee80211_key; just adopt it. */
...
static void eapol_input_group_msg1(uint8_t *frame, uint32_t length)
{
  ...
  if((ieee80211_encryption&0x0F) == IEEE80211_CIPHER_CCMP)
  {
	...
	/* The GTK unwrap below reads a fixed 32 bytes (MICK+EK) out of key_data;
	   make sure the frame is actually that long before touching it. */
	if((uint32_t)(fr_in->key_frame.key_data - frame)
	     + (EAPOL_MICK_LENGTH + EAPOL_EK_LENGTH) > length) {
		DBG_WIFI("GTK key_data too short, drop"EOL);
		return;
	}
