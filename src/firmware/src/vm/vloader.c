...
void loaderInit(uint8_t *src)
{
  ...
  /* The bytecode is copied into _vmem_heap (VMEM_LENGTH int32 words); a
     bogus/oversized size field would overflow that region. Refuse it. */
	if(n > (uint32_t)VMEM_LENGTH*4) {
		consolestr("loaderInit: bytecode too large, refusing"EOL);
		return;
	}
