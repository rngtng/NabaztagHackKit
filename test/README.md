...
`task test` preprocesses `test/test.mtl` and runs it in the MTL simulator.
`test/test.mtl` includes `test/helper.mtl` + `test/assertions.mtl`, then
`test/lib/_test.mtl` (all of `lib/`) and `test/native/_test.mtl` (VM-native
...
shadow earlier real implementations. Concretely, reading top to bottom:
...
3. The `lib/net` client transport/config stubs (`tcp_open`, `udp_send`,
   ...
   If you need to test something that only exists inside the ipv4 stack
   itself, write an isolated test program instead of extending this shared file.
...
6. `lib/forth.mtl` + `forth_run`/`forth_run_stack` helpers, which build the
   ...
   ~200K-word heap (see the `?OM Error` gotcha in `CLAUDE.md`).
...
2. If the module declares `proto` seams, add stubs for **all** of them to
   `test/lib/_test.mtl` before its `#include`, even ones you don't plan to
   call — an undefined proto is a hard compile failure (`X is EMPTY !!!`).
3. Add one `#include "test/lib/<subdir>/<module>_test.mtl"` line inside
   `fun lib_test=`.
4. Run `task test`; iterate against real assertion failures (`!!` lines) —
   the harness fails the process now (see `CLAUDE.md`), no manual log-reading.

## Assertions

`assert_equalI`, `assert_equalS`, `assert_nil`, `assert_equalIL`,
`assert_equalSL`, `assert_equalTL`, `assert_equal`/`assert_not_equal` (truthy
checks) — defined in `test/assertions.mtl`. `assert_equalS` breaks on strings
