 ...
 *
 * Exactly TWO names are needed to compile src/net/{ieee80211,eapol}.c against
 * this (checked with an empty stub: B_OHCIIRQ_MASK and HostCtl), so that is all
 * it models. A header mirroring the whole real one would rot silently; this one
 * fails to compile the moment something reaches for a register it does not have.
 *
 * This is the ONLY stub header the mtl net/ tests need - unlike the lua track,
 * which also shadows ml674061.h for its register-only HAL drivers. Neither
 * net/ source dereferences an SoC register, so sys/inc/ml674061.h is used
 * as-is here (verified: `nab_regs` never appears in the objects' undefined
 * symbols).
 *
