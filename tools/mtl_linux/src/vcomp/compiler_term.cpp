...
int Compiler::parseterm()
{
    ...
    else if (parser->token[0]=='"')	// gestion des chaines
    {
        ...
        if (k=addlabel(globals,"",val,STACKGET(m,0)))
        {
...
int Compiler::parsefields(intptr_t* p)
{
    ...
    if (k=copytype(VALTOPNT(TABGET(type,REF_TYPE))))
    {
        return k;    // création du type
    }

    int loop=1;
    while(loop)
    {
        if (k=copytype(VALTOPNT(TABGET(p,REF_TYPE))))
        {
            return k;    // création du type du champ
        }
...
int Compiler::parseref()
{
    ...
    if (val!=-1)
    {
        ...
        if (code>=0)	// appel d'une fonction
        {
            ...
            else if (k=STACKPUSH(m,TABGET(p,REF_TYPE)))
            {
                return k;
            }
            return unif_argfun();
        ...
        else if (code==CODE_VAR || code==CODE_CONST)	// lecture d'une référence
        {
            ...
            if (k=STACKPUSH(m,TABGET(p,REF_TYPE)))
            {
                return k;
            }
            return parsegetpoint();
...
int Compiler::parsegetpoint()
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if (k=copytype(VALTOPNT(TABGET(stdtypes,STDTYPE_fun__tab_u0_I__u0))))
            {
                return k;
            }
            if (k=unif_argfun())
...
int Compiler::parseif()
{
    ...
    if (k=parseexpression())
    {
        return k;    // lire l'expression 'then'
    }
    ...
    if ((parser->next(0))&&(!strcmp(parser->token,"else")))
    {
        if (k=parseexpression())
        {
            return k;    // lire l'expression 'else'
        }
    }
...
int Compiler::parsefor()
{
    ...
    if (k=parseexpression())
    {
        return k;    // lire la valeur d'initialisation
    }
    // [type_init name_it]
    int i=nblabels(locals);
    if (k=addlabel(locals,STRSTART(VALTOPNT(STACKGET(m,1))),INTTOVAL(i),STACKGET(m,0)))
    {
        return k;
    }
    if (i+1>nblocals)
    {
        nblocals=i+1;    // nombre maximum de variables locales
    }
    ...
    if (!strcmp(parser->token,";"))
    {
        ...
        if (k=parseexpression())
        {
            return k;    // lire la valeur itérée
        }
        bc->addchar(OPgoto);
...
int Compiler::parsematch()
{
    ...
    if (k=parseexpression())
    {
        return k;    // lire l'objet
    }
    if (k=createnodetype(TYPENAME_UNDEF))
    {
        return k;    // préparer le type du résultat
    }
    // [result src]

    if (k=parser->parsekeyword("with"))
    {
        return k;
    }

    intptr_t end;
    if (k=parsematchcons(&end))
    {
        return k;
    }
...
int Compiler::parsematchcons(intptr_t* end)
{
            ...
            &&(p=searchref(PNTTOVAL(newpackage),parser->token)))	// recherche dans les autres globales
    {
        ...
        if ((code==CODE_CONS)||(code==CODE_CONS0))
        {
            ...
            if (newnloc>nblocals)
            {
                nblocals=newnloc;    // nombre maximum de variables locales
            }
            ...
            // [type_src result src]
            if (k=unif(VALTOPNT(STACKGET(m,0)),VALTOPNT(STACKGET(m,2))))
            {
                return k;
            }
            STACKDROP(m);
...
intptr_t Compiler::parselet()
{
    ...
    if (k=parseexpression())
    {
        return k;    // lire l'expression du let
    }
    removenlabels(locals,nloctodelete);
    return 0;
}
    ...
    else if (!strcmp(parser->token,"("))
    {
        ...
        while(1)	// la liste est dans la pile
        {
            ...
            else if (islabel(parser->token))
            {
                ...
                if (k=addlabel(locals,parser->token,INTTOVAL(i),STACKGET(m,0)))
                {
                    return k;
                }
                if ((parser->next(0))&&(!strcmp(parser->token,")")))
                {
                    bc_byte_or_int(i,OPsetlocalb,OPsetlocal);
                    if (k=unif(VALTOPNT(STACKGET(m,0)),plist))
                    {
                        return k;
                    }
                    STACKDROPN(m,2);
                ...
                bc->addchar(OPfirst);
                bc_byte_or_int(i,OPsetlocalb,OPsetlocal);
                if (k=unif(VALTOPNT(STACKGET(m,0)),pval))
                {
                    return k;
                }
                STACKDROP(m);
...
int Compiler::parseset()
{
    ...
    if (!searchlabel_byname(locals,parser->token,&val,&ref))	// recherche dans les variables locales
    {
        ...
        if (k=parsesetpoint(1,VALTOINT(val),&opstore))
        {
            return k;
        }
    }
    else
    {
        ...
        if (val!=-1)
        {
            ...
            if (code==CODE_VAR)	// variable
            {
                ...
                if (k=parsesetpoint(0,VALTOINT(TABGET(p,REF_PACKAGE)),&opstore))
                {
                    return k;
                }
...
int Compiler::parsesetpoint(int local,int ind,int* opstore)
{
    ...
    if (local)
    {
        bc_byte_or_int(ind,OPgetlocalb,OPgetlocal);
    }
    else
    {
        bc_byte_or_int(ind,OPgetglobalb,OPgetglobal);
    }
    *opstore=OPstore;
    while(1)
    {
        ...
        if (ind>=0)
        {
            bc_byte_or_int(ind,OPfetchb,OPfetch);
        }
        else
        {
            bc->addchar(OPfetch);
...
int Compiler::parsepntfun()
{
    ...
    if (val!=-1)
    {
        ...
        {
            int v=0;
            if (TABGET(p,REF_PACKAGE)!=NIL)
            {
                v=VALTOINT(TABGET(p,REF_PACKAGE));
            }
            else
            {
    ...
    if ((parser->next(0))&&(!strcmp(parser->token,"[")))
    {
        ...
        while(1)
        {
            ...
            if (!strcmp(parser->token,"]"))
            {
                ...
                if (k=copytype(VALTOPNT(TABGET(stdtypes,STDTYPE_fun__fun_u0_u1_u0__u1))))
                {
                    return k;
                }
                return unif_argfun();
