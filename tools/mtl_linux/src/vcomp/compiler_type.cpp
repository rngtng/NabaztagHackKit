...
{
    int size=0;
    if ((type==TYPENAME_UNDEF)||(type==TYPENAME_WEAK))
    {
        size=0;
    }
    if ((type==TYPENAME_LIST)||(type==TYPENAME_TAB)||(type==TYPENAME_REC))
    {
        size=1;
    }
    if (type==TYPENAME_FUN)
    {
        size=2;
    }
...
intptr_t Compiler::createnodetuple(int size)
{
    ...
    for(i=size-1; i>=0; i--)
    {
        TABSET(m,p,TYPEHEADER_LENGTH+i,STACKPULL(m));
    }
    if (STACKPUSH(m,PNTTOVAL(p)))
    {
        return MTLERR_OM;
...
intptr_t Compiler::createnodetupleval(int size)
{
    ...
    for(i=size-1; i>=0; i--)
    {
        TABSET(m,p,TYPEHEADER_LENGTH+i,STACKGET(m,2*(size-1-i)));
    }
    if (STACKPUSH(m,PNTTOVAL(p)))
    {
        return MTLERR_OM;
...
int Compiler::parsegraph(Parser* p,int env,int mono,int rec,int labels,int newvars,intptr_t* rnode)
{
    ...
    int val;
    if (!searchlabel_byname(labels,p->token,&val,NULL))
    {
        return STACKPUSH(m,val);
    }
    ...
    else if ((p->token[0]=='r')&&(isdecimal(p->token+1)))
    {
        ...
        if (k=createnodetype(TYPENAME_REC))
        {
            return k;
        }
    ...
    else if (!strcmp(p->token,"tab"))
    {
        ...
        if (k=parsegraph(p,env,mono,rec+1,labels,newvars,rnode))
        {
            return k;
        }
    ...
    else if (!strcmp(p->token,"fun"))
    {
        ...
        if (newvars)
        {
            if (k=parsegraph(p,env,mono,rec+1,labels,1,rnode))
            {
                return k;
            }
            nblab=0;
    ...
    else if (islabel(p->token))
    {
        ...
        if (vargs==NIL)
        {
            return STACKPUSH(m,TABGET(t,REF_TYPE));
        }
        else
        {
            if (k=createnodetypecore(TABGET(q,TYPEHEADER_LENGTH+1)))
            {
                return k;
            }
...
intptr_t Compiler::parse_rnode(intptr_t *p)
{
    ...
    if ((c==TYPENAME_WEAK)||(c==TYPENAME_UNDEF))
    {
        return 0;    // type faible ou non défini, pas de récursion
    }
    if ((c==TYPENAME_CORE)&&(TABGET(p,TYPEHEADER_LENGTH)==NIL))
    {
        return 0;    // type de base, non paramétrique
    }
    ...
    if (c==TYPENAME_CORE)
    {
        ...
        for(i=0; i<TABLEN(tup); i++) if (k=parse_rnode(VALTOPNT(TABGET(tup,i))))
            {
                return k;
            }
    }
    ...
    {
        if (k=parse_rnode(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH))))
        {
            return k;
        }
        if (k=parse_rnode(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH+1))))
        {
            return k;
        }
    }
    ...
    {
        for(i=TYPEHEADER_LENGTH; i<TABLEN(p); i++) if (k=parse_rnode(VALTOPNT(TABGET(p,i))))
            {
                return k;
            }
    }
...
//k=parsegraph(p,env,mono,0,labels,0,&rnode);
    if (k=parsegraph(p,env,mono,0,labels,0,&rnode))
    {
        return k;
    }
    ...
    if (rnode)
    {
        return parse_rnode(VALTOPNT(STACKGET(m,0)));
    }
    return 0;
}
...
intptr_t Compiler::reccopytype(intptr_t *p)
{
    ...
    if (vq!=NIL)
    {
        return STACKPUSH(m,vq);    // élément déjà copié
    }
    int c=VALTOINT(TABGET(p,TYPEHEADER_CODE));
    if ((c==TYPENAME_CORE)&&(TABGET(p,TYPEHEADER_LENGTH)==NIL))
    {
    ...
    if (c==TYPENAME_TUPLE)
    {
        for(i=TYPEHEADER_LENGTH; i<TABLEN(p); i++) if (k=STACKPUSH(m,NIL))
            {
                return k;
            }
        if (k=createnodetuple(TABLEN(p)-TYPEHEADER_LENGTH))
        {
            return k;
        }
        ...
        for(i=TYPEHEADER_LENGTH; i<TABLEN(p); i++)
        {
            if (k=reccopytype(VALTOPNT(TABGET(p,i))))
            {
                return k;
            }
    ...
    if (c==TYPENAME_WEAK)
    {
        return STACKPUSH(m,PNTTOVAL(p));    // type faible, ne pas copier
    }

    if (c==TYPENAME_CORE)
    {
        k=createnodetypecore(TABGET(p,TYPEHEADER_LENGTH+1));
    }
    else
    {
        k=createnodetype(c);
    ...
    if (c==TYPENAME_FUN)
    {
        if (k=reccopytype(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH))))
        {
            return k;
        }
        TABSET(m,q,TYPEHEADER_LENGTH,STACKPULL(m));
        if (k=reccopytype(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH+1))))
        {
            return k;
        }
    ...
    else if (c==TYPENAME_CORE)
    {
        ...
        {
            intptr_t* tup=VALTOPNT(vargs);
            for(i=0; i<TABLEN(tup); i++) if (k=reccopytype(VALTOPNT(TABGET(tup,i))))
                {
                    return k;
                }
            if (k=DEFTAB(m,TABLEN(tup)))
            {
                return k;
            }
            TABSET(m,q,TYPEHEADER_LENGTH,STACKPULL(m));
...
intptr_t Compiler::recresetcopy(intptr_t *p)
{
    ...
    if (c==TYPENAME_WEAK)
    {
        return 0;    // type faible, pas de récursion
    }
    else if (c==TYPENAME_CORE)
    {
        int vargs=TABGET(p,TYPEHEADER_LENGTH);
        ...
        {
            intptr_t* tup=VALTOPNT(vargs);
            for(i=0; i<TABLEN(tup); i++) if (k=recresetcopy(VALTOPNT(TABGET(tup,i))))
                {
                    return k;
                }
        }
    ...
    {
        if (k=recresetcopy(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH))))
        {
            return k;
        }
        if (k=recresetcopy(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH+1))))
        {
            return k;
        }
    }
    ...
    {
        for(i=TYPEHEADER_LENGTH; i<TABLEN(p); i++) if (k=recresetcopy(VALTOPNT(TABGET(p,i))))
            {
                return k;
            }
    }
...
intptr_t Compiler::recgoweak(intptr_t *p)
{
    ...
    if (vq!=NIL)
    {
        return 0;    // élément déjà traité
    }
    int c=VALTOINT(TABGET(p,TYPEHEADER_CODE));
    if (c==TYPENAME_WEAK)
    {
        return 0;    // type faible, ne pas traiter
    }
    ...
    if (c==TYPENAME_CORE)
    {
        ...
        {
            intptr_t* tup=VALTOPNT(vargs);
            for(i=0; i<TABLEN(tup); i++) if (k=recgoweak(VALTOPNT(TABGET(tup,i))))
                {
                    return k;
                }
        }
    ...
    {
        if (k=recgoweak(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH))))
        {
            return k;
        }
        if (k=recgoweak(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH+1))))
        {
            return k;
        }
    }
    ...
    {
        for(i=TYPEHEADER_LENGTH; i<TABLEN(p); i++) if (k=recgoweak(VALTOPNT(TABGET(p,i))))
            {
                return k;
            }
    }
...
intptr_t Compiler::recunif(intptr_t* s,intptr_t* t)
{
    ...
    if (s==t)
    {
        return 0;    // ceci gère le cas des types basiques
    }
    ...
    else if ((ns==nt)&&(ns==TYPENAME_CORE)&&(TABGET(s,TYPEHEADER_LENGTH+1)!=TABGET(t,TYPEHEADER_LENGTH+1)))
    {
        ok=0;
    }
    if (!ok)
...
intptr_t Compiler::unif_argfun()
{
    ...
    if (k=unif(VALTOPNT(TABGET(fun,TYPEHEADER_LENGTH)),arg))
    {
        return k;
    }
...
{
    if (code==TYPENAME_CORE)
    {
        PRINTF(m)(LOG_COMPILER,"%s",STRSTART(VALTOPNT(TABGET(p,TYPEHEADER_LENGTH+1))));
    }
    else if (code==TYPENAME_UNDEF)
    {
        PRINTF(m)(LOG_COMPILER,"u*");
    }
    else if (code==TYPENAME_WEAK)
    {
        PRINTF(m)(LOG_COMPILER,"w*");
    }
    else if (code==TYPENAME_TAB)
    {
        PRINTF(m)(LOG_COMPILER,"tab");
    }
    else if (code==TYPENAME_LIST)
    {
        PRINTF(m)(LOG_COMPILER,"list");
    }
    else if (code==TYPENAME_TUPLE)
    {
        PRINTF(m)(LOG_COMPILER,"tuple");
    }
    else if (code==TYPENAME_FUN)
    {
        PRINTF(m)(LOG_COMPILER,"fun");
    }
}
...
intptr_t Compiler::recunifbigger(intptr_t* s,intptr_t* t)
{
    ...
    if (ns==nt)
    {
        ...
        else if (ns==TYPENAME_CORE)
        {
            ...
            if ((vtups==NIL)||(vtupt==NIL))
            {
                restoreactual(t,s,vt,vs,MTLERR_TYPE);
            }
            intptr_t* tups=VALTOPNT(vtups);
            intptr_t* tupt=VALTOPNT(vtupt);
            int len=TABLEN(tups);
