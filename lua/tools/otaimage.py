...
producing the `.sim` file the setup-page uploader (net/ota.lua) verifies before
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Stamp a raw firmware .bin into a .sim OTA image")
    ap.add_argument("input", help="raw firmware binary (e.g. bin/firmware.bin)")
    ap.add_argument("-o", "--output", required=True, help="output .sim path")
