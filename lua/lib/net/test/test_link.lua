...
-- aton is the non-throwing form: "not an address" is an answer, not an error
eq(link.aton("192.168.0.42"), H"c0a8002a", "link.aton dotted quad")
eq(link.aton("0.0.0.0"), H"00000000", "link.aton all-zero address")
eq(link.aton("255.255.255.255"), H"ffffffff", "link.aton broadcast")
eq(link.aton("example.com"), nil, "link.aton returns nil for a hostname")
eq(link.aton("192.168.0"), nil, "link.aton rejects 3 octets")
eq(link.aton("300.1.1.1"), nil, "link.aton rejects octet > 255")
eq(link.aton("1.2.3.4.5"), nil, "link.aton rejects 5 octets")
eq(link.aton(" 1.2.3.4"), nil, "link.aton is anchored")
eq(link.aton(nil), nil, "link.aton rejects a non-string")
