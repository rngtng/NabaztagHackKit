-- Host-side unit-test runner for lua/sys (#259): the wall clock and the SNTP
-- client, under the tools/luac host `lua` (same vendored tree + LUA_32BITS
-- luaconf.h as the device, so integer width and the 32-bit tick wrap match the
-- rabbit exactly).
--
-- Same shape as net/test/run.lua: module stdlib lint, eq/ok helpers, then
-- every test_*.lua for a landed module. Fixtures come from an independent
-- Python generator (scratchpad ntpfix.py): SNTP packets from struct.pack off
-- RFC 4330, civil dates from CPython's datetime, DST instants from the IANA
-- tzdb via zoneinfo - none of it derived from the modules under test. Every
-- assertion names the expected value per the repo testing rule.
--
-- Usage: lua sys/test/run.lua   (paths resolved relative to arg[0])
...
-- device-global stub: sys.time falls back to nab.time() when handed no tick
TICK = 0
nab = {time = function() return TICK end}
...
print(("sys tests: %d passed, %d failed"):format(passed, failed))
