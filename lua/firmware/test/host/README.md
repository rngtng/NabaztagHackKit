...
| `tone_test.c` | `inc/tone_midi.h` | #331 — `nab.tone()`'s resident asset, a **generated** 45-byte Standard MIDI File. Rebuilt here from the SMF spec and cross-checked against the constants `lib/audio/midi.lua` carries, so the codec swap that bought 2,112 B cannot rot into an invalid file. The only test whose subject is data rather than code, and it links no firmware source at all |
