...
| `lcframe_test.c` | `src/utils/lcframe.c` | the `#LC` frame checksum (#298) — pinned vectors so the C receiver and the two Python senders cannot drift, and every single-nibble flip in a chunk-sized buffer detected |
